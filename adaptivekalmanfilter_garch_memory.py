...
if __name__ == "__main__":
    ...
    f = KalmanFilter_memory(dim_x=3, dim_z=1, N=200)

    f.x = np.array([[1],[1],[1]])      

    f.A = np.array([[0.16, 0,0],
                   [0,0.994,0],
                   [0,0,-0.106]])    # state transition matrix

    f.H = np.array([[30.57, -34.61, 0.45]])    # Measurement function
    f.P *= 1000                  # covariance matrix
    f.R = 1.98                      # state uncertainty
    f.Q = np.array([[1.01, 0,0],
                   [0,1.10,0],
                   [0,0, 88432.96]])              # process uncertainty
    ...
    z = pd.read_csv('kf_garch.csv', header = 0)
    z=z['x']
    ...
    for t in range (len(z)):
        ...
        # save data
        results.append(dot(f.H,f.x))
        measurements.append(z[t])
    
    results = [res[0][0] for res in results]
    res = pd.DataFrame(results)
    res.to_csv('kf_garch_predicted_N_200.csv')